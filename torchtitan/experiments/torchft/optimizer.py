...
class _OptimizerAdapter:
    """Expose the base optimizer implementation without re-entering FT dispatch."""

    def __init__(
        self,
        owner: "TorchFTOptimizersContainer",
...
class TorchFTOptimizersContainer(OptimizersContainer):
    ...
    def __init__(
        ...
        *,
        ...
        self._inner_optimizer = self._create_optimizer_adapter()
        self._ft_optimizer = torchft.Optimizer(ft_manager.manager, self._inner_optimizer)
        # Whether to determine quorum using FT.optimizer,
        # in semi-sync training we use the synchronization step to start quorum
        # A single-replica quorum always commits, so the wrapper only adds
    ...
    def _create_optimizer_adapter(
        self, optimizer: Optimizer | None = None
    ) -> Optimizer:
        return cast(Optimizer, _OptimizerAdapter(self, optimizer))
    ...
    def step(self, *args, **kwargs) -> None:
        ...
        The inner adapter calls the base implementation directly so the wrapper
        cannot recursively enter this method.
        """
